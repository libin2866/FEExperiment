...
check_root(){
	[[ $EUID != 0 ]] && echo -e "${Error}  Current   Account 非ROOT(或 no ROOT权限)，无法继续操作，请使用${Green_background_prefix} sudo su ${Font_color_suffix}来获取临时ROOT权限（执行后会提示输入 Current   Account 的 password ）。" && exit 1
}
check_sys(){
...
SSR_installation_status(){
	[[ ! -e ${config_user_file} ]] && echo -e "${Error} can't find ShadowsocksR  Config  file ， please check  !" && exit 1
	[[ ! -e ${ssr_folder} ]] && echo -e "${Error} can't find ShadowsocksR  folder ， please check  !" && exit 1
}
Server_Speeder_installation_status(){
	[[ ! -e ${Server_Speeder_file} ]] && echo -e "${Error}  no  install  ruisu(Server Speeder)， please check  !" && exit 1
}
LotServer_installation_status(){
	[[ ! -e ${LotServer_file} ]] && echo -e "${Error}  no  install  LotServer， please check  !" && exit 1
}
BBR_installation_status(){
	if [[ ! -e ${BBR_file} ]]; then
		echo -e "${Error} can't find BBR script  ，Start Download..."
		cd "${file}"
		if ! wget -N --no-check-certificate https://raw.githubusercontent.com/ToyoDAdoubi/doubi/master/bbr.sh; then
			...
			echo -e "${Info} BBR  script  Downloadcomplete !"
			chmod +x bbr.sh
		fi
	fi
}
...
View_User(){
	...
	if [[ -z "${now_mode}" ]]; then
		...
		echo -e "  mixture \t    : ${Red_font_prefix}${obfs}${Font_color_suffix}"
 ...
 在浏览器中，打开 QRCode link ，就可以看到 QRCode 图片。
  protocol 和 mixture 后面的[ _compatible ]，指的是 兼容原版 protocol / mixture 。"
		echo && echo "==================================================="
	else
		user_total=`${jq_file} '.port_password' ${config_user_file} | sed '$d' | sed "1d" | wc -l`
		[[ ${user_total} = "0" ]] && echo -e "${Error} can't find 多Port User ， please check  !" && exit 1
		clear && echo "===================================================" && echo
		echo -e " ShadowsocksR Account  Config Info：" && echo
		echo -e " I  P\t    : ${Green_font_prefix}${ip}${Font_color_suffix}"
		echo -e " Encode\t    : ${Green_font_prefix}${method}${Font_color_suffix}"
...
Set_config_password(){
	echo "Please Input ShadowsocksR Account  Password"
...
Set_config_method(){
	echo -e "please choose ShadowsocksR Account  encryption method

 ${Green_font_prefix} 1.${Font_color_suffix} none
 ${Tip} if you use auth_chain_a  protocol ，the encryption method choose none， mixture by you like (suggest plain)
...
Set_config_obfs(){
 ...
 ${Tip} 如果使用 ShadowsocksR 加速游戏，请选择  mixture 兼容原版或 plain  mixture ，然后客户端选择 plain，否则会增加延迟 !
 另外, 如果你选择了 tls1.2_ticket_auth，那么客户端可以选择 tls1.2_ticket_fastauth，这样即能伪装又不会增加延迟 !
 如果你是在日本、美国等热门地区搭建，那么选择 plain  mixture 可能被墙几率更低 !" && echo
	...
	echo && echo ${Separator_1} && echo -e "	 mixture  : ${Green_font_prefix}${ssr_obfs}${Font_color_suffix}" && echo ${Separator_1} && echo
	if [[ ${ssr_obfs} != "plain" ]]; then
			read -e -p "if use Config confusing plugin (_compatible)？[Y/n]" ssr_obfs_yn
...
Set_config_protocol_param(){
	while true
	do
	echo -e "Please Input 要Config的ShadowsocksR Account  欲 limit 的 deviceNum  (${Green_font_prefix} auth_* 系列 protocol  不兼容原版才有效 ${Font_color_suffix})"
...
Set_config_speed_limit_per_con(){
	while true
	do
	echo -e "Please Input 要Config的每个Port   single  thread   speed limit 上限( single 位：KB/S)"
	echo -e "${Tip}  single  thread  speed limit ：每个Port   single  thread 的 speed limit 上限，多 thread 即无效。"
...
Set_config_speed_limit_per_user(){
	...
	do
	echo
	echo -e "Please Input to Config every Port  all speed   speed limit(unit：KB/S)"
	echo -e "${Tip} Port all  speed limit ：Every Port  all speed   speed limit ，single Port whole speed limit 。"
...
Check_python(){
	...
	if [[ -z ${python_ver} ]]; then
		echo -e "${Info}  no  install Python，Start  install ..."
		if [[ ${release} == "centos" ]]; then
...
Download_SSR(){
	...
	[[ ! -e "manyuser.zip" ]] && echo -e "${Error} ShadowsocksR server  压缩包 Download failed  !" && rm -rf manyuser.zip && exit 1
	unzip "manyuser.zip"
	[[ ! -e "/usr/local/shadowsocksr-manyuser/" ]] && echo -e "${Error} ShadowsocksR server   unzip  failed  !" && rm -rf manyuser.zip && exit 1
	mv "/usr/local/shadowsocksr-manyuser/" "/usr/local/shadowsocksr/"
	[[ ! -e "/usr/local/shadowsocksr/" ]] && echo -e "${Error} ShadowsocksR server  重命名 failed  !" && rm -rf manyuser.zip && rm -rf "/usr/local/shadowsocksr-manyuser/" && exit 1
	rm -rf manyuser.zip
	[[ -e ${config_folder} ]] && rm -rf ${config_folder}
	mkdir ${config_folder}
	[[ ! -e ${config_folder} ]] && echo -e "${Error} ShadowsocksR Config  file 的 folder  建立 failed  !" && exit 1
	echo -e "${Info} ShadowsocksR server  Downloadcomplete !"
}
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/ToyoDAdoubi/doubi/master/service/ssr_centos -O /etc/init.d/ssr; then
			echo -e "${Error} ShadowsocksR service   manage  script  Download failed  !" && exit 1
		fi
	...
	echo -e "${Info} ShadowsocksR service   manage  script  Downloadcomplete !"
}
# Install JQ解析器
JQ_install(){
	if [[ ! -e ${jq_file} ]]; then
		...
		[[ ! -e ${jq_file} ]] && echo -e "${Error} JQ解析器 重命名 failed ， please check  !" && exit 1
		chmod +x ${jq_file}
		echo -e "${Info} JQ解析器  install complete，继续..."
	else
...
Installation_dependency(){
	...
	[[ ! -e "/usr/bin/unzip" ]] && echo -e "${Error}  deps  unzip( unzip 压缩包)  install  failed ，多半是软件包源的问题， please check  !" && exit 1
	Check_python
...
Install_SSR(){
	check_root
	[[ -e ${config_user_file} ]] && echo -e "${Error} ShadowsocksR  Config  file 已存在， please check ( 如 install  failed 或者存在旧版本，请先卸载 ) !" && exit 1
	[[ -e ${ssr_folder} ]] && echo -e "${Error} ShadowsocksR  folder 已存在， please check ( 如 install  failed 或者存在旧版本，请先卸载 ) !" && exit 1
	echo -e "${Info} Start Config ShadowsocksR Account  Config ..."
	Set_config_all
	echo -e "${Info} Start  install / Config  ShadowsocksR deps..."
	Installation_dependency
	...
	JQ_install
	echo -e "${Info} Start 写入 ShadowsocksR Config  file ..."
	Write_configuration
	echo -e "${Info} Start Config iptables.."
	Set_iptables
	echo -e "${Info} Start Add  iptables rules..."
	Add_iptables
	echo -e "${Info} Start Save  iptables rules..."
	Save_iptables
	echo -e "${Info} 所有步骤  install 完毕，Start 启动 ShadowsocksR server ..."
	Start_SSR
}
Update_SSR(){
	SSR_installation_status
	echo -e "因破娃暂停更新ShadowsocksR server ，所以此功能临时禁用。"
	#cd ${ssr_folder}
	#git pull
	#Restart_SSR
}
Uninstall_SSR(){
	[[ ! -e ${config_user_file} ]] && [[ ! -e ${ssr_folder} ]] && echo -e "${Error}  no  install  ShadowsocksR， please check  !" && exit 1
	echo "sure to uninstall ShadowsocksR？[y/N]" && echo
	...
	if [[ ${unyn} == [Yy] ]]; then
		...
		echo && echo " ShadowsocksRuninstall complete !" && echo
	else
		echo && echo "uninstall 已 cancel ..." && echo
	fi
}
Check_Libsodium_ver(){
	echo -e "${Info} Start get libsodium latest version..."
	Libsodiumr_ver=$(wget -qO- "https://github.com/jedisct1/libsodium/tags"|grep "/jedisct1/libsodium/releases/tag/"|head -1|sed -r 's/.*tag\/(.+)\">.*/\1/')
	[[ -z ${Libsodiumr_ver} ]] && Libsodiumr_ver=${Libsodiumr_ver_backup}
	echo -e "${Info} libsodium latest version ${Green_font_prefix}${Libsodiumr_ver}${Font_color_suffix} !"
}
Install_Libsodium(){
	if [[ -e ${Libsodiumr_file} ]]; then
		echo -e "${Error} libsodium has installed  ,sure to overwrite install (Update)？[y/N]"
	...
	[[ ! -e ${Libsodiumr_file} ]] && echo -e "${Error} libsodium  install  failed  !" && exit 1
...
debian_View_user_connection_info(){
	...
	if [[ -z "${now_mode}" ]]; then
		...
		user_list_all="Port: ${Green_font_prefix}"${user_port}"${Font_color_suffix}\t link IP total : ${Green_font_prefix}"${user_IP_total}"${Font_color_suffix}\t  Current  link IP: ${Green_font_prefix}${user_IP}${Font_color_suffix}\n"
		user_IP=""
		...
		do
			...
			user_list_all=${user_list_all}"Port: ${Green_font_prefix}"${user_port}"${Font_color_suffix}\t link IP total : ${Green_font_prefix}"${user_IP_total}"${Font_color_suffix}\t  Current  link IP: ${Green_font_prefix}${user_IP}${Font_color_suffix}\n"
			user_IP=""
		done
...
Modify_Config(){
	SSR_installation_status
	if [[ -z "${now_mode}" ]]; then
		echo && echo -e " Current Mode :  single Port ， What do you want ？
 ${Green_font_prefix}1.${Font_color_suffix} Change  User Port
 ${Green_font_prefix}2.${Font_color_suffix} Change  User  password
 ${Green_font_prefix}3.${Font_color_suffix} Change  encryption way
 ...
 ${Green_font_prefix}7.${Font_color_suffix} Change   single  thread  speed limit
 ...
 ${Green_font_prefix}9.${Font_color_suffix} Change   All  Config " && echo
		...
		echo && echo -e " Current Mode : 多Port ， What do you want ？
 ${Green_font_prefix}1.${Font_color_suffix}  Add  User  Config
 ${Green_font_prefix}2.${Font_color_suffix}   delete  User  Config
 ${Green_font_prefix}3.${Font_color_suffix}  Change  User  Config
——————————
 ${Green_font_prefix}4.${Font_color_suffix}  Change  encryption way
 ...
 ${Green_font_prefix}8.${Font_color_suffix}  Change   single  thread  speed limit
...
Add_multi_port_user(){
	...
	Add_iptables
	Save_iptables
	echo -e "${Info} multi Port User Add complete ${Green_font_prefix}[Port: ${ssr_port} , Password: ${ssr_password}]${Font_color_suffix} "
}
...
Modify_multi_port_user(){
	...
	if [[ ! -z "${del_user}" ]]; then
		...
		Add_iptables
		Save_iptables
		echo -e "${Inof} multi Port User Change complete ${Green_font_prefix}[旧: ${modify_user_port}  ${password} , 新: ${ssr_port}  ${ssr_password}]${Font_color_suffix} "
	else
		echo -e "${Error} Please Input right Port  !" && exit 1
	fi
}
#  delete  多Port User  Config
Del_multi_port_user(){
	List_multi_port_user
	user_total=`${jq_file} '.port_password' ${config_user_file} | sed '$d' | sed "1d" | wc -l`
	[[ "${user_total}" = "1" ]] && echo -e "${Error} multi Port User only left 1个，cannot delete!" && exit 1
	echo -e "Please Input to delete User Port "
	...
	if [[ ! -z ${del_user} ]]; then
		...
		echo -e "${Info} multi Port User delete complete ${Green_font_prefix} ${del_user_port} ${Font_color_suffix} "
	else
		echo "${Error} Please Input right Port  !" && exit 1
	fi
}
...
View_Log(){
	SSR_installation_status
	[[ ! -e ${ssr_log_file} ]] && echo -e "${Error} ShadowsocksRlog file doesn't exist !" && exit 1
...
Install_ServerSpeeder(){
	...
	[[ ! -e "serverspeeder.sh" ]] && echo -e "${Error} ruisu install script Download failed !" && exit 1
...
Uninstall_ServerSpeeder(){
	...
	if [[ ${unyn} == [Yy] ]]; then
		...
		echo && echo "ruisu(Server Speeder) uninstall complete !" && echo
	fi
}
# LotServer
Configure_LotServer(){
...
Install_LotServer(){
	...
	[[ ! -e "/tmp/appex.sh" ]] && echo -e "${Error} LotServer  install  script  Download failed  !" && exit 1
	bash /tmp/appex.sh 'install'
	sleep 2s
	PID=`ps -ef |grep -v grep |grep "appex" |awk '{print $2}'`
	if [[ ! -z ${PID} ]]; then
		echo -e "${Info} LotServer  install complete !" && exit 1
	else
		echo -e "${Error} LotServer  install failed !" && exit 1
	fi
}
Uninstall_LotServer(){
...
1.  install 开启BBR，需要更换内核，存在更换 failed 等风险(重启后无法开机)
2. 本 script  仅支持 Debian / Ubuntu 系统更换内核，OpenVZ和Docker 不支持更换内核
...
Install_BBR(){
	[[ ${release} = "centos" ]] && echo -e "${Error} 本 script  不支持 CentOS系统 install  BBR !" && exit 1
	BBR_installation_status
	bash "${BBR_file}"
}
Start_BBR(){
...
Other_functions(){
  ...
  Notice： ruisu/LotServer/BBR doesn't support OpenVZ！
...
Set_config_connect_verbose_info(){
	...
	if [[ ${connect_verbose_info} = "0" ]]; then
		echo && echo -e " Current  log Mode: ${Green_font_prefix} Simple Mode  模式（只输出错误log）${Font_color_suffix}" && echo
		echo -e " Sure to switch to  ${Green_font_prefix} Detail Mode （输出详细连接log+错误log）${Font_color_suffix}？[y/N]"
		...
		echo && echo -e " Current  log模式: ${Green_font_prefix} Detail Mode （输出详细连接log+错误log）${Font_color_suffix}" && echo
		echo -e " Sure to switch to  ${Green_font_prefix} Simple Mode  模式（只输出错误log）${Font_color_suffix}？[y/N]"
		read -e -p "(Default: n):" connect_verbose_info_ny
		[[ -z "${connect_verbose_info_ny}" ]] && connect_verbose_info_ny="n"
		if [[ ${connect_verbose_info_ny} == [Yy] ]]; then
...
Update_Shell(){
	...
	echo -e " script  is the latest[ ${sh_new_ver} ] !(Notice：因为更新 way为直接覆盖 Current  运行的 script  ，所以可能下面会提示一些报错，无视即可)" && exit 0
}
# Display 菜 single Status
menu_status(){
	if [[ -e ${config_user_file} ]]; then
		check_pid
		if [[ ! -z "${PID}" ]]; then
			echo -e "  Current  Status: ${Green_font_prefix}has installed ${Font_color_suffix} and ${Green_font_prefix}start${Font_color_suffix}"
		else
			echo -e "  Current  Status: ${Green_font_prefix}has installed ${Font_color_suffix} but ${Red_font_prefix}not start${Font_color_suffix}"
		fi
		now_mode=$(cat "${config_user_file}"|grep '"port_password"')
		if [[ -z "${now_mode}" ]]; then
			echo -e "  Current Mode : ${Green_font_prefix} single Port ${Font_color_suffix}"
		else
			echo -e "  Current Mode : ${Green_font_prefix}multi Port ${Font_color_suffix}"
		fi
	else
		echo -e "  Current  Status: ${Red_font_prefix}uninstalled ${Font_color_suffix}"
	fi
}
check_sys
[[ ${release} != "debian" ]] && [[ ${release} != "ubuntu" ]] && [[ ${release} != "centos" ]] && echo -e "${Error} 本 script  doesen't support Current system ${release} !" && exit 1
echo -e "  ShadowsocksR onekey manage  script   ${Red_font_prefix}[v${sh_ver}]${Font_color_suffix}
...
————————————
  ...
  ${Green_font_prefix}6.${Font_color_suffix} Display connected Info
  ${Green_font_prefix}7.${Font_color_suffix} Config User  Config
  ${Green_font_prefix}8.${Font_color_suffix} mannual Change  Config
...
————————————
 ...
 "
menu_status
echo && read -e -p "Please Input Num [1-15]：" num
case "$num" in
	1)
	Install_SSR
	;;
